...
class AdminUserCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$username) {
            $username = $io->ask('Username', 'admin', function (string $value): string {
                if (trim($value) === '') throw new \RuntimeException('O username não pode ser vazio.');
                if (strlen($value) < 3) throw new \RuntimeException('O username deve ter pelo menos 3 caracteres.');
                return $value;
            });
        }
        ...
        if (!$password) {
            ...
            $io->askHidden('Confirmar senha', function (string $confirm) use ($password): string {
                if ($confirm !== $password) throw new \RuntimeException('As senhas não conferem.');
                return $confirm;
            });
        }
    ...
    private function listUsers(SymfonyStyle $io): void
    {
        ...
        $io->table(['ID', 'Username', 'Nome', 'E-mail', 'Roles', 'Criado em'], $rows);
